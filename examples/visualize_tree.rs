...
use vaabbit::{physics::{dynamictree::DynamicTree, quadtree::{Node, QuadTree}}, shapes::*};
...
async fn main() {
    ...
    for i in 0..300 {
        ...
        let size = Vec2::new(rand::gen_range(1.0, 5.0), rand::gen_range(1.0, 5.0));
    ...
    loop {
        ...
        rand::srand(4);
        //draw_node(&tree.root);
        let debug_info = tree.get_debug_info();
        println!("debug info: {:?}", tree.nodes.len());
        for (is_leaf, aabb) in debug_info {
            if is_leaf == 1 {
                draw_aabb(aabb, Color::new(0.5, 0.8, 0.5, 1.0));
                continue;
            }
            draw_aabb(aabb, Color::new(0.5, 0.5, 0.5, 0.5));
        ...
        let t = Instant::now();
        let mut overlaps = tree.query(&test);
